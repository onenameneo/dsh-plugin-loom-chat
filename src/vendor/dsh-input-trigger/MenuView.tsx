...
import type { MenuViewInjected } from './slots.js'
...
function stableStore<T>(store: { getSnapshot: () => T; subscribe: (listener: () => void) => () => void }) {
  ...
  return {
    ...
    getSnapshot: () => {
      ...
      if (!initialized || !snapshotsEqual(previous, next)) {
        previous = next
        initialized = true
      }
...
function snapshotsEqual(left: unknown, right: unknown): boolean {
  ...
  if (left instanceof Map && right instanceof Map) {
    ...
    return [...left.entries()].every(([key, value], index) => {
      ...
      return Object.is(key, rightKey) && snapshotsEqual(value, rightValue)
    })
  }
  ...
  if (typeof left === 'object' && left !== null && typeof right === 'object' && right !== null) {
    ...
    return keys.length === Object.keys(rightRecord).length
      && keys.every(key => snapshotsEqual(leftRecord[key], rightRecord[key]))
  }
  return false
}
...
export function MenuView({ menu, headers, onPick, onCrumb, onHover, onDismiss, t }: MenuViewProps) {
  const stableMenu = useMemo(() => stableStore(menu), [menu])
  const stableHeaders = useMemo(() => stableStore(headers), [headers])
  const state = useSyncExternalStore(
    stableMenu.subscribe,
    stableMenu.getSnapshot,
  )
  const crumbs = useSyncExternalStore(
    stableHeaders.subscribe,
    stableHeaders.getSnapshot,
  )
          ...
          : (
            <Fragment key={group.source}>
              ...
              {group.showGroupTitle === false || group.items.some(item => item.section !== undefined)
                ? null
                : <div className={css.groupTitle} role="presentation" data-source={group.source}>{t(group.source as never)}</div>}
...
/** Canvas adapter for the official session-scoped InputTrigger controller. */
export function menuFaceOf(controller: {
  menu: MenuViewInjected['menu']
  headers: MenuViewInjected['headers']
  pick: (source: string, index: number, action?: any) => void
